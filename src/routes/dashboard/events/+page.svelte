...
{:else}
	<h2 class="my-4 text-2xl">No of Events: {data.eventList?.length}</h2>

	<div class="mt-8 mb-4 p-0 pt-4 lg:w-full lg:p-0">
		<FilterMenu
			bind:filteredList
			data={data?.eventList}
			filterKeys={['entryType', 'format', 'isLive', 'isPublished']}
		/>
		<DataTable data={filteredList} {columns} fileName="Events List" />
	</div>
{/if}
